...
export default function SecureNotesApp() {
      ...
      try {
        if (isRegistering) {
          ...
          const salt = response.salt
            ? sodium.from_base64(response.salt)
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
        <main className="bg-gray-800 rounded-lg shadow-xl p-6 md:p-8 w-full max-w-md" role="main">
          ...
          <div
            className="bg-yellow-900/50 border border-yellow-600 rounded-lg p-4 mb-6"
            role="note"
  ...
  const NotesList = () => {
    ...
    return (
      <nav
        className="w-full md:w-80 bg-gray-800 md:border-r border-gray-700 flex flex-col h-full"
        role="navigation"
          ...
          {notesError ? (
            ...
            filteredNotes.map((note) => (
              ...
              >
                ...
                <p className="text-sm text-gray-400 line-clamp-2">{note.content || 'No content'}</p>
  ...
  const AppLayout = () => {
    return (
      <div className="h-screen flex flex-col md:flex-row bg-gray-900">
        ...
        <div
          className={`${currentView === 'notes' && !selectedNote && currentView !== 'editor' ? 'hidden md:flex' : 'flex'} flex-1 flex-col`}
        >
          <header className="hidden md:flex bg-gray-800 border-b border-gray-700 px-6 py-3 items-center justify-between">
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2" role="status" aria-live="polite">
                <div
                  className={`w-2 h-2 rounded-full ${encryptionStatus === 'unlocked' ? 'bg-green-500' : 'bg-red-500'}`}
                  aria-hidden="true"
                />
