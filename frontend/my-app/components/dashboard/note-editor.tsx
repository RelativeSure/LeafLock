...
export function NoteEditor() {
  const { selectedNote, updateNote, moveToTrash, selectNote, tags, createTag } = useNotes()
    ...
    if (selectedNote && displayContent !== undefined) {
      const timeoutId = setTimeout(async () => {
        try {
          ...
          updateNote(selectedNote.id, { title, content: contentToSave, tags: noteTags })
        } catch (err) {
  ...
  const handleToggleEncryption = async () => {
    ...
    if (!selectedNote.encrypted) {
      ...
      try {
        ...
        updateNote(selectedNote.id, { encrypted: true, content: encryptedContent })
      } catch (err) {
        console.error("[v0] Failed to encrypt:", err)
      }
    } else {
      // Disabling encryption
      updateNote(selectedNote.id, { encrypted: false, content: displayContent })
    }
  }
  ...
  const handleCreateAndAddTag = () => {
    if (newTagName.trim()) {
      const tag = createTag({ name: newTagName.trim() })
