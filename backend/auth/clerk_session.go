...
func (sm *ClerkSessionManager) ValidateAndRefreshSession(c *fiber.Ctx, token string) (*jwt.Claims, error) {
	ctx := c.Context()
...
func (sm *ClerkSessionManager) GetCurrentSessionInfo(ctx context.Context, token string) (*SessionInfo, error) {
	...
	return &SessionInfo{
		...
		PublicMeta:   claims.PublicMetaData,
		PrivateMeta:  claims.PrivateMetaData,
	}, nil
}
...
type SessionInfo struct {
	...
	ExpiresAt    time.Time
	IsAdmin      bool
	PublicMeta   map[string]interface{}
