...
import AdminUserPicker from '@/features/admin/components/AdminUserPicker'
import {
  Select,
  SelectContent,
  SelectItem,
...
const AdminPage: React.FC<AdminPageProps> = ({ api, onBack }) => {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        // ignore registration fetch failures; admin view still usable
      }
    })()
  }, [
    q,
  ...
  const refreshSelectedRoles = async () => {
    ...
    try {
      ...
      setError('Unable to refresh user roles')
    }
  }
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center justify-between rounded-md border p-3">
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">
                    Toggle public sign-ups for new users
                  </div>
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <Button variant="secondary" onClick={assignRole} disabled={!selected}>
                    Assign
                  </Button>
                  <Button variant="secondary" onClick={removeRole} disabled={!selected}>
                    Remove
                  </Button>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Card>
                    ...
                    <CardContent className="space-y-1 text-sm text-muted-foreground">
                      ...
                      <div>
                        Last login:{' '}
                        {selected.last_login ? new Date(selected.last_login).toLocaleString() : '-'}
                      </div>
                    </CardContent>
                  </Card>
                  <Card>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {bulkAction === 'assign'
                ? 'Assign role to all filtered users'
                : 'Remove role from all filtered users'}
            </DialogTitle>
            <DialogDescription>
              This will {bulkAction} the role “{bulkRole}” for all users matching current filters (
              {total} users). Proceed?
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
            <Label htmlFor="confirm-bulk-role" className="text-sm">
              Type CONFIRM to continue
            </Label>
            <Input
              id="confirm-bulk-role"
              value={confirmRoleText}
              onChange={(e) => setConfirmRoleText(e.target.value)}
              placeholder="CONFIRM"
            />
          </div>
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setStatus(
                    `${bulkAction === 'assign' ? 'Assigned' : 'Removed'} for ${res.affected ?? 'selected'} users`
                  )
                  setBulkOpen(false)
                  await loadUsers()
                } catch (e: any) {
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {adminAction === 'grant'
                ? 'Grant admin to all filtered users'
                : 'Revoke admin for all filtered users'}
            </DialogTitle>
            <DialogDescription>
              This will {adminAction} admin for all users matching current filters ({total} users).
              Proceed?
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
            <Label htmlFor="confirm-bulk-admin" className="text-sm">
              Type CONFIRM to continue
            </Label>
            <Input
              id="confirm-bulk-admin"
          ...
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => setBulkAdminOpen(false)} disabled={busy}>
              Cancel
            </Button>
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setStatus(
                    `${adminAction === 'grant' ? 'Granted' : 'Revoked'} admin for ${res.affected ?? 'selected'} users`
                  )
