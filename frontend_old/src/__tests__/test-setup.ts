// Vitest global mocks are intentionally permissive while restructuring; revisit to hard-type later.
...
// Prevent MaxListenersExceededWarning during test runs
...
class MockFileReader implements FileReader {
  ...
  readAsArrayBuffer = vi.fn<(file: Blob) => void>()
  readAsBinaryString = vi.fn<(file: Blob) => void>()
}
...
class MockNotification extends EventTarget {
  ...
  constructor(title: string, options: NotificationOptions = {}) {
    ...
    if (options.vibrate) {
      this.vibrate = Array.isArray(options.vibrate)
        ? Object.freeze([...options.vibrate])
        : undefined
    }
  }
