...
<CardGrid columns={2}>
  ...
  <Card title="PostgreSQL" icon="database">
    Provisioned via container `postgres` with data volume `leaflock_postgres_data`. Schema is auto-created on boot; see `/architecture/database-schema` for table definitions.
  </Card>
...
All environment variables are documented in `/getting-started/environment-variables`. For day-to-day work ensure the following are set:
...
LeafLock enforces a zero-knowledge policy even locally, so password resets and key rotation rely on the same encryption workflows as production 🎯.
...
CI executes the same targets via the unit-test, lint, coverage, and Lighthouse workflows before publishing container images. Keep the pipeline green and updates sail through 🚀.

## Further Reading

- `/api/rest-api` for endpoint semantics and request schemas
- `/architecture/encryption` for client cryptography internals
