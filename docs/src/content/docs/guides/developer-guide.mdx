...
- Admin access is managed through Clerk user metadata
- Open the UI and try the admin endpoints (requires Clerk authentication)
...
- Admin access is managed through Clerk user metadata
- Set `is_admin: true` in Clerk user metadata via API or dashboard
...
The consolidated `leaflock.sh` helper has been retired. Use direct commands for day-to-day tasks:

```bash
docker compose up -d --build   # Start local stack
docker compose down            # Stop local stack
make -C backend test           # Backend tests
pnpm -C frontend run lint      # Frontend lint
pnpm -C frontend run test      # Frontend tests
./scripts/dev-setup.sh         # One-time developer onboarding helper
```

### Available Scripts

- `scripts/dev-setup.sh` — Developer onboarding (env scaffolding, tooling checks)
- `scripts/backup.sh` — Manual PostgreSQL backup helper for ops teams
- `frontend/scripts/check-no-jsx.sh` — Blocks accidental JSX in plain `.ts` files
...
Located at `.github/workflows/ci-frontend-lighthouse.yml`

**What it does:**
- Boots the stack with Docker Compose for realistic network and asset timing
- Runs the production frontend build
- Executes Lighthouse CI audits against the preview server
- Uploads reports as workflow artifacts for manual inspection

**Admin panel in Lighthouse runs:**
...
<Aside type="tip">
See the [Performance guide](/operations/performance) for detailed configuration and monitoring information.
</Aside>
