...
**Frontend**: **Do NOT set `VITE_API_URL`** in Railway environment variables. The frontend auto-discovers the PR backend via Railway's service discovery.

**Common Issue**: If PR frontend shows CORS errors connecting to production backend (`https://leaflock.app`), remove `VITE_API_URL` from Railway environment variables and redeploy.

See [Railway PR Preview Troubleshooting](/troubleshooting/railway/#railway-pr-preview-deployments) for detailed configuration.
...
- **PR CORS errors** → See [Railway PR Preview Deployments](#pr-preview-deployments) above.
