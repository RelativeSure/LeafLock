---
title: Railway
description: Deploy LeafLock on Railway with IPv6 support, private networking, and production-ready configuration.
---
...
# Private network (IPv6) - backend is private-only
frontend: leaflock-frontend.railway.internal
backend:  motivated-energy.railway.internal  # Railway-generated, internal only
```

**Security Note:** The backend service should NOT have a public domain configured. All API access goes through the frontend's Caddy reverse proxy.
...
# CORS for Railway (frontend domains only - backend has no public access)
...
```bash
# Internal proxy target (private mesh) - backend is NOT publicly accessible
BACKEND_INTERNAL_URL=http://leaflock-backend.railway.internal:8080

# Browser makes requests to frontend, which proxies to backend
VITE_API_URL=/api/v1
...
The frontend entrypoint now auto-detects Railway's service discovery. Supplying `BACKEND_INTERNAL_URL` and `VITE_API_URL` ensures:

- Caddy proxies `/api` traffic across Railway's private mesh to the backend
- Browser requests stay within the frontend domain (no direct backend access)
- Backend remains private-only, reducing attack surface
...
For troubleshooting 502 errors, see: [Railway Troubleshooting Guide](/troubleshooting/railway/)
...
3. **Smoke-test the stack**
   ...
   curl https://leaflock-frontend-production.up.railway.app/api/v1/health
   ```
...
```bash
# Backend Communication (private network only)
...
# Backend health check (through frontend proxy - backend is private-only)
curl https://leaflock-frontend-production.up.railway.app/api/v1/health
