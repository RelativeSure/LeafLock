...
description: Step-by-step instructions for deploying LeafLock on Coolify v4+ with IPv6 support and security best practices.
---
...
<CardGrid>
  <Card title="Coolify Instance" icon="server">
    Running Coolify v4.0+ with Docker support
  </Card>
  <Card title="Domain" icon="globe">
    A domain name pointed to your Coolify server
  </Card>
  <Card title="SSL Certificate" icon="lock">
    Coolify can auto-generate SSL certificates via Let's Encrypt
  </Card>
  <Card title="Git Repository" icon="github">
    Access to this LeafLock repository
  </Card>
</CardGrid>
...
<Tabs>
  ...
  <TabItem label="Application">
    ...
    # User registration (secure default: false)
    ENABLE_REGISTRATION=false  # Set to true to enable public registration
...
:::note[Security Note]
Always use strong, randomly generated passwords and keys for production deployments. Never commit sensitive environment variables to version control.
:::