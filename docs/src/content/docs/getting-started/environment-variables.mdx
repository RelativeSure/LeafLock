...
**Required**: Yes
**Type**: String
**Example**: `pk_test_Y2xlcmsuaW5mb3JtYXRpb24tNzguY2xlcmsuY29tJA`
**Description**: Clerk publishable key for backend API calls. Get from Clerk dashboard → API Keys.
...
**Required**: Yes
**Type**: String
**Example**: `sk_test_1reFhwQ0FIOw3yksdhi3F7j8qhd98y2I7eqhd98y2I7e`
**Description**: Clerk secret key for backend authentication. Keep secure and never expose publicly. Get from Clerk dashboard → API Keys.
...
**Default**: `development`
**Example**: `production`
**Description**: Environment mode. When set to `development`, automatically allows `*.railway.app` origins for PR testing and relaxes CSRF cookies to `SameSite=Lax`/`Secure=false` for http://localhost testing. `production` enforces strict CSRF cookies (`Secure`, `SameSite=Strict`) and requires explicit `CORS_ORIGINS`.
...
**Development Mode**: When `APP_ENV=development`, `*.railway.app` is automatically allowed for PR deployments.
...
**Default**: `false`
**Example**: `true`
**Description**: Seeds `app_settings.registration_enabled` during startup. When `false` (default), registration requests return a generic acknowledgement without creating an account—preventing attackers from probing which emails exist. Administrators can explicitly enable registration later in the UI or database when onboarding users.
**Security Note**: Leave `false` for production. Toggle to `true` only during a controlled signup window, then revert.
...
# Server encryption for sensitive data
