...
This document provides comprehensive technical details about how Clerk authentication is integrated into LeafLock, including code examples, architecture decisions, and migration strategies.
...
graph TB
    ...
    subgraph "Backend"
        E[Dual Auth Middleware] --> F{Token Type?}
...
2. **Backend Validation**
   - Dual authentication middleware receives request
   - Attempts Clerk token validation first
   - Falls back to JWT validation if Clerk fails
...
```go title="backend/auth/dual_middleware.go"
...
During migration, Clerk user IDs need to be mapped to internal user IDs:
...
func (h *Handler) getOrCreateUserFromClerk(ctx context.Context, clerkUserID string, email string, isAdmin bool) (uuid.UUID, error) {
...
For existing users, password migration requires careful handling:

```go
func (h *Handler) initiateUserMigration(ctx context.Context, userID uuid.UUID, email string) error {
    ...
    if err != nil {
        return fmt.Errorf("failed to store migration token: %w", err)
    }
    ...
    return h.emailService.SendTemplate(ctx, email, "migration_invite", emailData)
}
```
...
type AuthMetrics struct {
    ...
    MigrationAttempts  prometheus.Counter
    MigrationSuccesses prometheus.Counter
}
...
func (h *Handler) recordAuthMetrics(authType string, success bool) {
    ...
    if success {
        if authType == "clerk" {
            ...
            h.metrics.JWTValidations.With(labels).Inc()
        }
    } else {
