...
LeafLock includes **Swagger/OpenAPI** documentation for interactive API exploration:

- **Swagger UI**: `/api/v1/docs` - Interactive API browser with "Try it out" functionality
- **OpenAPI Spec**: `/api/v1/docs/openapi.json` - Machine-readable API specification
...
|------|-----------|-------------|
| `note_update` | Server → Client | Note content changed by collaborator |
| `cursor_position` | Bidirectional | Real-time cursor position for collaboration |
| `user_joined` | Server → Client | User joined collaboration session |
| `user_left` | Server → Client | User left collaboration session |
| `presence` | Client → Server | Heartbeat to maintain connection |
